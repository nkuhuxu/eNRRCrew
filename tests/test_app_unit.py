...
def test_chat_result_updates_artifacts_and_trace(monkeypatch, tmp_path: Path) -> None:
    streamlit = fake_streamlit()
    streamlit.chat_input.return_value = "predict yield"
    monkeypatch.setattr(app_module, "st", streamlit)
    managed = AgentRunResult(
        status="completed",
        answer="Yield result",
        strategy=RouteStrategy.FAST_PATH,
        detected_routes=[ConversationRoute.YIELD_PREDICTION],
    ...
    monkeypatch.setattr(
        app_module,
        "ConversationService",
        lambda: SimpleNamespace(answer=lambda *args: managed),
    )
    monkeypatch.setattr(app_module, "_agent_context", lambda *args: object())
    monkeypatch.setattr(app_module, "_active_datasets", lambda *args: {"curated": tmp_path})
    settings = SimpleNamespace(api_key="key")
    workspace = SimpleNamespace(root=tmp_path)

    app_module.render_chat(settings, workspace)

    assert streamlit.session_state["chat_messages"][-1]["content"] == "Yield result"
    assert streamlit.session_state["result_yield"]["category"] == "High"
    assert streamlit.session_state["csv_code_editor"].endswith("print('draft')")
...
def test_chat_empty_success_and_failure_retrieval(monkeypatch, tmp_path: Path) -> None:
    ...
    managed = AgentRunResult(
        ...
        answer="evidence",
        strategy=RouteStrategy.FAST_PATH,
        detected_routes=[ConversationRoute.RETRIEVAL],
        rounds=1,
    )
    monkeypatch.setattr(
        app_module,
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            answer=lambda *args: (_ for _ in ()).throw(RuntimeError("offline"))
        ),
    )
    app_module.render_chat(settings, workspace)
    assert "Coordination failed" in streamlit.session_state["chat_messages"][-1]["content"]


def test_offline_chat_guides_explicit_tasks_to_safe_pages() -> None:
    assert "Yield prediction tab" in app_module._offline_chat_guidance(
        "predict NH3 yield"
    )
    assert "Catalyst recommendation tab" in app_module._offline_chat_guidance(
        "推荐催化剂"
    )
    assert "Configure an API key" in app_module._offline_chat_guidance(
        "Explain catalyst tradeoffs"
    )
...
def test_main_dispatches_all_workspaces(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(app_module, "render_knowledge_update", MagicMock())
    app_module.main()
    ...
    app_module.render_knowledge_update.assert_called_once_with(settings, workspace)
