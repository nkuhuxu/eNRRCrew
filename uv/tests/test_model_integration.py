...
@pytest.mark.integration
def test_lancedb_014_can_open_legacy_graphrag_store() -> None:
    stores = sorted((ASSET_ROOT / "output").glob("*/artifacts/lancedb"))
    if not stores:
        pytest.skip("legacy GraphRAG LanceDB store is unavailable")
    database = lancedb.connect(str(stores[-1]))
